...
ScrollMagic.Controller = function (options) {
   ...
   var construct = function () {
      ...
      _isDocument =
         ...
         !document.body.contains(_options.container); // normalize to window
      if (_isDocument) {
         _options.container = window;
   ...
   var updateScenes = function () {
      if (_enabled && _updateScenesOnNextCycle) {
         ...
         var scenesToUpdate = _util.type.Array(_updateScenesOnNextCycle)
            ...
            : _sceneObjects.slice(0); // reset scenes
   ...
   }); // (BUILD) - REMOVE IN MINIFY - END // for scenes we have getters for each option, but for the controller we don't, so we need to make it available externally for plugins
